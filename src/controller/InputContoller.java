...
public class InputContoller {
    ...
    public boolean checkItemStock() {
        ...
        if(!itemPresence.validate(items)){
            output.add("One of the Item is incorrect. It is not present in Database");
        }else if(!itemStock.validate(items)){
            output.add("Item is out of stock. Please write correct quantities of items");
        }else if(!itemCategory.validate(items)){
            output.add("Limit on one of the Categories has crossed limit. Please Make sure quantities of Essential items should be maximum 3, Luxury items should be maximum 4 and Misc items should be maximum 6");
        }
